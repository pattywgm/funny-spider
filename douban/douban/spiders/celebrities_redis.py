...
"""
@version: 1.0
@file: celebrities_redis.py
@time: 17/10/23  下午6:12
@desc: 影人抓取分布式版本,采用scrapy-redis
"""
import random
import re
...
from douban.utils.my_utils import random_delay, get_redis_conn
...
class Celebrities(RedisCrawlSpider):
    name = 'celebrities_r'
    meta_version = _META_VERSION
    redis_key = "celebrities:start_urls"

    def __init__(self, *args, **kwargs):
        super(Celebrities, self).__init__(*args, **kwargs)
        self.r = get_redis_conn()
    ...
    def parse(self, response):
        ...
        item = CelebrityItem()
        item['url'] = url
        item['celebrity_code'] = re.findall('\d+', url)[0]
        # 名字,头像
    ...
    def parse_partners(self, response):
        ...
        if len(partner_divs) > 0 and start<=10:
            start += 10
